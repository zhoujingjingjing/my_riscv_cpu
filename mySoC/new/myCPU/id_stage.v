...
module id_stage(
    ...
    // [测试框架完善] 每拉高一拍表示流水线确实因 load-use 冒险停顿一周期。
  ...
  /*............input bus from IF stage.............*/
    ...
    assign id_to_exe_bus = {
        ...
        // [RV32M移植] 只把“这是M指令”和原始funct3送入EXE。
        // funct3本身正好就是rv32m_mdu的八种操作编号，不再重复编码。
        is_rv32m,
        mdu_op
    };
    ...
    // [RV32M移植] 八条命名信号只在ID级用于清晰译码；跨级仍传1位类别和3位funct3。
    ...
    // [RV32M移植] RV32M八条指令共用R型opcode=0110011、funct7=0000001。
    // 先保留八个独立的指令名，便于阅读源码和在波形中直接观察具体指令。
   ...
   // [测试框架完善] 两个源寄存器可在同一拍同时相关，但流水线只停一拍，
   // 因此必须先逻辑或再计数；被异常/mret冲刷的年轻指令不计入性能事件。
   assign perf_load_use_stall = (rf_rdata1_hazard || rf_rdata2_hazard) && id_valid
                                && !wb_flush;
    /*  ld.w  r1,  8(r2)
        add.w r3,  r1, r4
