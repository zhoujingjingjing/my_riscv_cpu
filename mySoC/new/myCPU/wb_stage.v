...
module wb_stage(
    ...
    assign wb_to_id_bus = {
        wb_valid,
        // [第4步验证修复] WB是气泡时，wb_reg里仍保留上一条指令的reg_we。
        // 写使能必须与wb_valid相与，防止CSR刚写入后又用新CSR值重复覆盖rd。
        wb_reg_we && wb_valid,
        wb_reg_waddr,
    ...
    assign debug_wb_dnpc = wb_dnpc;
    assign debug_wb_mem_valid = wb_valid && wb_mem_valid;
    assign debug_wb_mem_store = wb_mem_store;
    assign debug_wb_mem_addr = wb_mem_addr;
    assign debug_wb_mem_mask = wb_mem_mask;
    assign debug_wb_mem_wdata = wb_mem_wdata;
    assign debug_wb_mem_rdata = wb_mem_rdata;
    assign debug_wb_trap_valid = wb_ex;
    assign debug_wb_trap_cause = wb_cause;
