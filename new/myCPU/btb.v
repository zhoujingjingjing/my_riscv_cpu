// 文件职责：分支目标缓冲器、方向预测器和返回地址栈（BTB/BHT/RAS）。
// 上游：IF级提供当前PC，ID/EXE提供调用返回信息和真实分支结果。
// 下游：IF级使用预测目标；EXE的真实结果在预测错误时更新本模块。
`timescale 1ns / 1ps
`include "mycpu_top.h"
...
module btb #(
    ...
    // [阶段1新增] 第二个只读查询口，专门查询同组PC+4。
    // 它与A口共享同一份64项BTB和同一次EXE更新，不复制预测状态。
    input  wire [31:0] if_pc_b,
    output wire        pre_taken_b,
    output wire [31:0] pre_target_b,
    output wire        pre_is_ret_b,
    output wire [INDEX_LEN-1:0] pre_index_b,
    /*==========================================================*/
    ...
    // [阶段1新增] B口与A口使用完全相同的Index/Tag切分和命中规则。
    wire [INDEX_LEN-1:0] if_idx_b = if_pc_b[INDEX_LEN+1 : 2];
    wire [TAG_LEN-1:0]   if_tag_b = if_pc_b[TAG_LEN+INDEX_LEN+1 : INDEX_LEN+2];
    
    // 命中(match)的条件必须同时满足：这行不是空的 (valid为1) 且 这行记录的Tag和当前正在取的PC的Tag一模一样。
    wire match = btb_valid[if_idx] && (btb_tag[if_idx] == if_tag);
    wire match_b = btb_valid[if_idx_b] && (btb_tag[if_idx_b] == if_tag_b);
    
    // 如果命中了，且表里记录这恰好是一条用于“函数返回”的指令，则置起 is_ret_hit。
    wire is_ret_hit = match && btb_is_ret[if_idx]; 
    wire is_ret_hit_b = match_b && btb_is_ret[if_idx_b];

    // 输出跳不跳结论：必须命中，且（是返回指令，100%跳；或者是普通跳转指令，且2位状态机的最高位是1，这表示倾向于跳）。
    assign pre_taken = match && (btb_is_ret[if_idx] || btb_counter[if_idx][1]);
    // [阶段1新增] 两个查询口都是纯组合读；EXE仍只有一个时序写口。
    assign pre_taken_b = match_b &&
                         (btb_is_ret[if_idx_b] || btb_counter[if_idx_b][1]);
    ...
    assign pre_target_b = is_ret_hit_b ? current_ras_top : btb_target[if_idx_b];
    
    // 把当拍算出来的索引和性质同步送给流水线，记账用。
    assign pre_is_ret = is_ret_hit;
    assign pre_index  = if_idx;
    assign pre_is_ret_b = is_ret_hit_b;
    assign pre_index_b = if_idx_b;
    /*==========================================================*/
