...
module miniRV_SoC (
    ...
    // ====================================================================
    wire        accel_sel = perip_en && (perip_addr[31:8] == 24'h80_2000);
    ...
    always @(posedge fpga_clk) begin
        if (fpga_rst) begin
            ...
            cop_load_we    <= 1'b0;
            if (accel_sel && (perip_we != 4'b0)) begin
                case (perip_addr[7:0])
                    8'h60: begin   // PUSH
                        case (cop_word_cnt)
    ...
    Accelerator u_accel (
        ...
        .STATE(accel_state),
        .load_start(cop_load_start), .load_we(cop_load_we),
        .load_addr(cop_load_addr), .load_wdata(cop_load_wdata),
        .load_done(cop_load_done), .calc_done(accel_calc_done),
        .result_data(),                       // 旧128位口不用
        .result_raddr(cop_result_row),
    ...
    wire [3:0]  dram_we = accel_sel ? 4'b0 : perip_we;
